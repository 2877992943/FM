...
def initial_para():
    ...
    n,d=xmatrix.shape
    w0=0.0
    w1=np.random.uniform(low=-0.5,high=0.5,size=(1,d))
    v_mat=np.random.uniform(low=-0.5,high=0.5,size=(d,v_k))
...
def train():
    ...
    for ep in range(epoch):
        ...
        for i in obs_list[:]:#[2,1,6,3...]
            ...
            v_g=v_g*grad_loss_f;#print 'v',v_g[:4,:4]
            ####normalize grad
...
if __name__=='__main__':
    ...
    parapath="D://python2.7.6//MachineLearning//FFM_CTR//para_fm.pkl"
    import pickle
    global w0,w1,v_mat,xmatrix
    with open(parapath,'wb')as f:
        w0=pickle.load(f)
        w1=pickle.dump(f)
        v_mat=pickle.dump(f)
        xmatrix=pickle.dump(f)
        xvec=pickle.dump(f)
